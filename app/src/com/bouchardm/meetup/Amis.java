...
public class Amis extends Activity {
	...
	public boolean onContextItemSelected(android.view.MenuItem item) {
		...
		if (type == ExpandableListView.PACKED_POSITION_TYPE_CHILD) {
			View ajoutGroupe = getLayoutInflater().inflate(
					R.layout.amis_ajout_groupe, null);
			EditText txtGroupe = (EditText) ajoutGroupe
					.findViewById(R.id.btnAjoutGroupeText);

			BtnSetHandlerChangerAmiGroupe handlerHoraire = new BtnSetHandlerChangerAmiGroupe(
					txtGroupe, group, child);
			...
			case R.id.menu_changerGroupe:
				// On demande à l'utilisateur dans quel groupe il veut envoyé
				// sont ami
				new AlertDialog.Builder(this)
						.setTitle("Entrez le nom du groupe")
						.setView(ajoutGroupe)
						.setNegativeButton("Annuler", null)
						.setPositiveButton("Changer", handlerHoraire).show();
			...
			case R.id.menu_modifier:
				...
				BtnSetHandlerChangerNomGroupe handlerHoraire = new BtnSetHandlerChangerNomGroupe(
						txtGroupe, group);
	...
	public ArrayList<ListeGroupeModel> SetStandardGroups() {
		...
		listeAmi.add(new ListeAmiModel("Jeff la patate test", null));
	...
	public void ajoutGroupe(View source) {
		...
		new AlertDialog.Builder(this).setTitle("Ajouté un groupe")
				.setView(ajoutGroupe).setNegativeButton("Annuler", null)
				.setPositiveButton("Ajouter", handlerHoraire).show();
	}
	...
	public void ajoutAmi(View source) {
		...
		new AlertDialog.Builder(this).setTitle("Ajouter un ami")
				.setView(ajoutGroupe).setNegativeButton("Annuler", null)
				.setPositiveButton("Ajouter", handlerHoraire).show();
	}
	...
	public class BtnSetHandlerAmi implements DialogInterface.OnClickListener {
		...
		public void onClick(DialogInterface dialog, int which) {
			...
			ArrayList<ListeAmiModel> toutLesAmis = ExpListItems.get(0)
					.getItems();
			for (ListeAmiModel ami : toutLesAmis) {
				// On rajoute tout les amis du groupe dans la liste d'ami (pour
				// ne pas les perdre)
				listeAmi.add(ami);
			}

			// Création d'un nouvel ami
			ListeAmiModel nouvelAmi = new ListeAmiModel(m_txtGroupe.getText()
					.toString(), null);
	 ...
	 * 
	 */
	public class BtnSetHandlerChangerNomGroupe implements
		 ...
		 * @param p_txtHoraire
		 */
		public BtnSetHandlerChangerNomGroupe(EditText p_txtGroupe,
				int ancienGroupe) {
			this.m_txtGroupe = p_txtGroupe;
		...
		public void onClick(DialogInterface dialog, int which) {
			...
			if (!nomGroupe.equalsIgnoreCase("Demande d'amitié")
				...
				Toast.makeText(getApplicationContext(),
						"Vous ne pouvez pas mettre ce nom de groupe",
	 ...
	 * 
	 */
	public class BtnSetHandlerChangerAmiGroupe implements
			DialogInterface.OnClickListener {
		/**
		 * Attributs
		 */
		 ...
		 * @param p_txtHoraire
		 */
		public BtnSetHandlerChangerAmiGroupe(EditText p_txtGroupe,
				int ancienGroupe, int ancienAmi) {
			this.m_txtGroupe = p_txtGroupe;
			this.m_ancienGroupe = ancienGroupe;
		...
		public void onClick(DialogInterface dialog, int which) {
			...
			if (!nomGroupe.equalsIgnoreCase("Demande d'amitié")
					&& !nomGroupe.trim().equalsIgnoreCase("")) {
				ListeAmiModel amiTempo = ExpListItems.get(m_ancienGroupe)
						.getItem(m_ancienAmi);
				...
				for (ListeGroupeModel expandListGroup : toutLesGroupes) {
					// Si le group est le même que celui entré on met
					// l'utilisateur dans ce groupe
					if (expandListGroup.getName().toString()
							.equalsIgnoreCase(nomGroupe)
							&& !trouve) {

						ArrayList<ListeAmiModel> listeAmiTempo = expandListGroup
								.getItems();
				...
				// Si on n'a pas trouver le groupe on le rajoute et on met
				// l'user dedans
				if (!trouve) {
					listeGroupe = new ArrayList<ListeGroupeModel>();
					listeAmi = new ArrayList<ListeAmiModel>();
				...
				Toast.makeText(getApplicationContext(),
						"Vous ne pouvez pas mettre votre ami dans ce groupe",
						Toast.LENGTH_LONG).show();
			}
		}
	}
	...
	public class ExpandListAdapter extends BaseExpandableListAdapter {
		...
		public ExpandListAdapter(Context context,
				ArrayList<ListeGroupeModel> groups) {
		...
		public Object getChild(int groupPosition, int childPosition) {
			ArrayList<ListeAmiModel> chList = groups.get(groupPosition)
					.getItems();
		...
		public View getChildView(int groupPosition, int childPosition,
			...
			ListeAmiModel child = (ListeAmiModel) getChild(groupPosition,
					childPosition);
			if (view == null) {
